...
public class StatusBarUtils {
    ...
    public static void setColor(Activity activity, int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            LogUtil.info("StatusUtil", "addColor: " + decorView.getChildCount());
            LogUtil.info("StatusUtil", "id: " + statusView.getId());
     ...
     * @param r
     */
    public static void setBackGroundResource(Activity activity, int r) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            // 设置状态栏透明
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            // 生成一个状态栏大小的矩形
            View statusView = createStatusBarViewResource(activity, r);
            // 添加 statusView 到布局中
            ViewGroup decorView = (ViewGroup) activity.getWindow().getDecorView();
            LogUtil.info("StatusUtil", "addColor: " + decorView.getChildCount());
            LogUtil.info("StatusUtil", "id: " + statusView.getId());
            if (decorView.getChildAt(decorView.getChildCount() - 1).getId() != Integer.valueOf(1)) {
                ...
                decorView.getChildAt(decorView.getChildCount() - 1).setBackgroundResource(r);
            }
            LogUtil.info("StatusUtil", "addColor: " + decorView.getChildCount());
    ...
    public static void cleanColor(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            ViewGroup decorView = (ViewGroup) activity.getWindow().getDecorView();
            LogUtil.info("StatusUtil", "cleanColor: " + decorView.getChildCount());
...
//            // 设置根布局的参数
    ...
    private static View createStatusBarViewResource(Activity activity, int color) {
        // 绘制一个和状态栏一样高的矩形
        View statusBarView = new View(activity);
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
        statusBarView.setLayoutParams(params);
    ...
    private static View createStatusBarView(Activity activity, String color) {
        ...
        statusBarView.setId(Integer.valueOf(1));
        LogUtil.info("StatusUtil", "id: " + statusBarView.getId());
