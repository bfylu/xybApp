package cn.payadd.majia.updateapk;
...
public class ApkDownloadService extends Service {
    ...
    private final String apkName = "/xyb.apk";
    ...
    static class MyHandler extends Handler {
        private final WeakReference<ApkDownloadService> mService;

        MyHandler(ApkDownloadService service) {
            mService = new WeakReference<ApkDownloadService>(service);
        }

        @Override
        public void handleMessage(Message msg) {
            ApkDownloadService service = mService.get();
            if (service != null) {
                switch (msg.what) {
                    case 1:
                        int rate = msg.arg1;
//                        service.mApplication.setIsDownload(true);
                        if (rate < 100) {
                            ...
                            // 下载完毕后变换通知形式
//                            Debug.error("ApkDownloadService apk 下载完成");
...
//                                    "application/vnd.android.package-archive");
                            ...
                            if(Build.VERSION.SDK_INT >= 24){
                                Uri uri = FileProvider.getUriForFile(
                                        service,
                                        service.getPackageName() + ".fileprovider",
                                        apkfile);
...
//								"下载完成", "文件已下载完毕", contentIntent);
    ...
    private void installApk() {
        ...
        if(Build.VERSION.SDK_INT >= 24){
            Uri uri = FileProvider.getUriForFile(
                    mApplication,
                    mApplication.getPackageName() + ".fileprovider",
                    apkfile);
...
//        Debug.error(Tag,"onCreate");
        mApplication = (BaseApplication) BaseApplication.getAppContext();
...
//        Debug.error(Tag,"onStartCommand");
        if (intent!=null&&intent.getExtras()!=null){
            ...
            Log.i(Tag,"url=" + apkUrl + "savePath=" + savePath);
        }
