...
public class SpaceDecoration extends RecyclerView.ItemDecoration {
    ...
    private int publicCount = Integer.MAX_VALUE;
    private boolean mPaddingEdgeSide = true;
    private boolean mPaddingStart = true;
    private boolean mPaddingHeaderpublic = false;
    ...
    public void setPaddingHeaderpublic(boolean mPaddingHeaderpublic) {
        this.mPaddingHeaderpublic = mPaddingHeaderpublic;
    }
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
         ...
         * 普通Item的尺寸
         */
        if ((position>=headerCount&&position<parent.getAdapter().getItemCount()-publicCount)){
            int gravity;
        ...
        }else {//只有Headerpublic进到这里
                if (mPaddingHeaderpublic) {//并且需要padding Header&public
                    if (orientation == OrientationHelper.VERTICAL){
                        if (mPaddingEdgeSide) {
                            outRect.left = halfSpace * 2;
                            outRect.right = halfSpace * 2;
