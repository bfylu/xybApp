...
public class ScrollLayout extends FrameLayout {
    ...
    private void initFromAttributes(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs,
                com.yinglan.scrolllayout.R.styleable.ScrollLayout);
        if (a.hasValue(com.yinglan.scrolllayout.R.styleable.ScrollLayout_maxOffset)) {
            int maxset = a.getDimensionPixelOffset(com.yinglan.scrolllayout.R.styleable.ScrollLayout_maxOffset, maxOffset);
            if (maxset != getScreenHeight()) {
                maxOffset = getScreenHeight() - maxset;
            }
        }
        if (a.hasValue(com.yinglan.scrolllayout.R.styleable.ScrollLayout_minOffset))
            minOffset = a.getDimensionPixelOffset(com.yinglan.scrolllayout.R.styleable.ScrollLayout_minOffset, minOffset);
        if (a.hasValue(com.yinglan.scrolllayout.R.styleable.ScrollLayout_exitOffset)) {
            int exitset = a.getDimensionPixelOffset(com.yinglan.scrolllayout.R.styleable.ScrollLayout_exitOffset, getScreenHeight());
            if (exitset != getScreenHeight()) {
                exitOffset = getScreenHeight() - exitset;
            }
        }
        if (a.hasValue(com.yinglan.scrolllayout.R.styleable.ScrollLayout_allowHorizontalScroll))
            isAllowHorizontalScroll = a.getBoolean(com.yinglan.scrolllayout.R.styleable.ScrollLayout_allowHorizontalScroll, true);
        if (a.hasValue(com.yinglan.scrolllayout.R.styleable.ScrollLayout_isSupportExit))
            isSupportExit = a.getBoolean(com.yinglan.scrolllayout.R.styleable.ScrollLayout_isSupportExit, true);
        if (a.hasValue(com.yinglan.scrolllayout.R.styleable.ScrollLayout_mode)) {
            int mode = a.getInteger(com.yinglan.scrolllayout.R.styleable.ScrollLayout_mode, 0);
            switch (mode) {
                case 0x0:
                    setToOpen();
                    break;
     ...
     * @param recyclerView
     */
    public void setAssociatedRecyclerView(MyScrollRecyclerView recyclerView) {
        recyclerView.getRecyclerView().addOnScrollListener(associatedRecyclerViewListener);
        updateRecyclerViewScrollState(recyclerView.getRecyclerView());
    }
