    ...
    public void showMessage(final ChatAdapter.ViewHolder viewHolder, final Context context) {
        ...
        switch (message.status()){
            ...
            case SendSucc:
                ...
                if (FileUtil2.isCacheFileExist(e.getFileId())){
                    showSnapshot(viewHolder,BitmapFactory.decodeFile(FileUtil2.getCacheFilePath(e.getFileId()), new BitmapFactory.Options()));
                }else{
                    snapshot.getImage(FileUtil2.getCacheFilePath(e.getFileId()), new TIMCallBack() {
                        @Override
                ...
                if (!FileUtil2.isCacheFileExist(fileName)) {
                    e.getVideo().getVideo(FileUtil2.getCacheFilePath(fileName), new TIMCallBack() {
                        @Override
                        public void onError(int i, String s) {
